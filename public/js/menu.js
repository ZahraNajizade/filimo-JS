    ...
    menuItem = res.menu.map((item) => {
      ...
      if (item.submenu.length > 0) {
        ...
        submenuItem = item.submenu
          .map((element) => {
            return `
          <li class="w-[50%]"><a href="#" class="py-[10px] px-[7px] block ">${element}</a></li>
          `;
          })
          .join("");
      }
      ...
      return `
      <li class="ml-[32px] relative">
                  <a href="#" class="gap-[4px] py-[14px] flex items-center text-[1.1rem]">
                    <span class="">${item.icon}</span>
                    <span class="">${item.menuItem}</span>
                    ${arrowDown}
                  </a>
                  <div class="absolute bg-[#1a1a1a] top-[55px] right-0 rounded-[5px] min-w-[333px] hidden">
